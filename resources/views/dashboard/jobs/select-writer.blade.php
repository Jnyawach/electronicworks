    ...
    <section>
        <div >
            <h5 class="fw-bold mb-3">Select a Writer to assign-({{$project->sku}})
                <a href="{{route('jobs.index')}}" class="btn-sm btn-primary hire text-decoration-none  float-end">SKIP & LET WRITERS SUBMIT BID</a>
            </h5>
                ...
                @if($writers->count()>0)
                    @foreach($writers as $writer)
                        <tr>
                            ...
                            <td>
                                <form id="{{$writer->id}}" action="{{route('selected-writer',$project->id)}}" method="POST">
                                    @method('PATCH')
                                    @csrf
