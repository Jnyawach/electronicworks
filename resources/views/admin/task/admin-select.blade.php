    ...
    <section>
        <div >
            <h5 class="fw-bold mb-3">Select a Writer to assign-({{$project->sku}})
                <a href="{{route('task.index')}}" class="btn-sm btn-primary hire text-decoration-none  float-end">SKIP & LET WRITERS SUBMIT BID</a>
            </h5>
                            ...
                            @foreach($writers as $writer)
                                <tr>
                                   ...
                                   ->getUrl('avatar_card'):'/images/no-image.png' )}}" class="img-fluid " style="height: 40px;">
                                    ...
                                    <td>
                                        <form id="{{$writer->id}}" action="{{route('admin-selecting',$project->id)}}" method="POST">
                                            @method('PATCH')
                                            @csrf
